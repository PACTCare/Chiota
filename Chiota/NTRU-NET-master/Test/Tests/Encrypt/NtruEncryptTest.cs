...
{
    /// <summary>
    /// Test the validity of the NtruEncrypt implementation
    /// </summary>
    public class NtruEncryptTest : ITest
    {
        #region Constants
        private const string DESCRIPTION = "Test the validity of the NtruEncrypt implementation";
        private const string FAILURE = "FAILURE! ";
        private const string SUCCESS = "SUCCESS! NtruEncrypt tests have executed succesfully.";
        #endregion
        ...
        public string Test()
        {
            ...
            {
                EncryptDecrypt();
                OnProgress(new TestEventArgs("Passed encryption and decryption"));
                EncodedKeysSlow();
                OnProgress(new TestEventArgs("Passed slow key encoding"));
                EncodedKeysFast();
                OnProgress(new TestEventArgs("Passed fast key encoding"));
                GenerateKeyPair();
                OnProgress(new TestEventArgs("Passed key generation"));
                GetOutputLength();
                OnProgress(new TestEventArgs("Passed parameter output length"));
                DigestTest();
                OnProgress(new TestEventArgs("Passed the alternating digest tests"));
        ...
        {
            // test each param set //
            EncryptDecrypt((NTRUParameters)NTRUParamSets.APR2011439.DeepCopy());
            OnProgress(new TestEventArgs("Passed the APR2011439 encryption test"));
            EncryptDecrypt((NTRUParameters)NTRUParamSets.APR2011439FAST.DeepCopy());
            OnProgress(new TestEventArgs("Passed the APR2011439FAST encryption test"));

            EncryptDecrypt((NTRUParameters)NTRUParamSets.APR2011743.DeepCopy());
            OnProgress(new TestEventArgs("Passed the APR2011743 encryption test"));
            EncryptDecrypt((NTRUParameters)NTRUParamSets.APR2011743FAST.DeepCopy());
            OnProgress(new TestEventArgs("Passed the APR2011743FAST encryption test"));

            EncryptDecrypt((NTRUParameters)NTRUParamSets.EES1087EP2.DeepCopy());
            OnProgress(new TestEventArgs("Passed the EES1087EP2 encryption test"));
            EncryptDecrypt((NTRUParameters)NTRUParamSets.EES1087EP2FAST.DeepCopy());
            OnProgress(new TestEventArgs("Passed the EES1087EP2FAST encryption test"));

            EncryptDecrypt((NTRUParameters)NTRUParamSets.EES1171EP1.DeepCopy());
            ...
            EncryptDecrypt((NTRUParameters)NTRUParamSets.EES1499EP1.DeepCopy());
        ...
        private void EmptyTest(NTRUEncrypt ntru, NTRUKeyPair kp, NTRUParameters param)
        {
            byte[] plainText = ByteUtils.GetBytes("");
            ntru.Initialize(kp.PublicKey);
            byte[] encrypted = ntru.Encrypt(plainText);
            ntru.Initialize(kp);
            byte[] decrypted = ntru.Decrypt(encrypted);
        ...
        private void EncodedKeysFast()
        {
            byte[] plainText = ByteUtils.GetBytes("secret encrypted text");

            NTRUParameters param = (NTRUParameters)NTRUParamSets.APR2011743FAST.DeepCopy();
            NTRUEncrypt ntru = new NTRUEncrypt(param);
