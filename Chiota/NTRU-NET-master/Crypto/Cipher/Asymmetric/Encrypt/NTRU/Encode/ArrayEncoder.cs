...
namespace VTDev.Libraries.CEXEngine.Crypto.Cipher.Asymmetric.Encrypt.NTRU.Encode
{
  ...
  public class ArrayEncoder
  {
    ...
    public static int[] DecodeMod3Sves(byte[] Data, int N, bool SkipFirst)
    {
      ...
      var coeffIndex = SkipFirst ? 1 : 0;
      var i = 0;

      while (i < Data.Length / 3 * 3 && coeffIndex < N - 1)
      {
        // process 24 bits at a time in the outer loop
        var chunk = (Data[i++] & 0xFF) | ((Data[i++] & 0xFF) << 8) | ((Data[i++] & 0xFF) << 16);
        for (var j = 0; j < 8 && coeffIndex < N - 1; j++)
        {
          // process 3 bits at a time in the inner loop
          var coeffTableIndex = ((chunk & 1) << 2) + (chunk & 2) + ((chunk & 4) >> 2);   // low 3 bits in reverse order
    ...
    {
      var size = (int)Math.Ceiling(N * Math.Log(3) / Math.Log(2) / 8);
      var arr = ArrayEncoder.ReadFullLength(InputStream, size);
    ...
    {
      var coeffs = new int[N];
      var bitsPerCoeff = 31 - IntUtils.NumberOfLeadingZeros(Q);
      var mask = IntUtils.URShift(-1, (32 - bitsPerCoeff));   // for truncating values to bitsPerCoeff bits
      var byteIndex = 0;
      var bitIndex = 0;       // next bit in data[byteIndex]
      var coeffBuf = 0;       // contains (bitIndex) bits
      var coeffBits = 0;      // length of coeffBuf
      var coeffIndex = 0;     // index into coeffs
    ...
    {
      var qBits = 31 - IntUtils.NumberOfLeadingZeros(Q);
      var size = (N * qBits + 7) / 8;
      var arr = ArrayEncoder.ReadFullLength(InputStream, size);
    ...
    {
      var numBits = (Data.Length * 3 + 1) / 2;
      var numBytes = (numBits + 7) / 8;
      var data = new byte[numBytes];
      var byteIndex = 0;
      var start = SkipFirst ? 1 : 0;
      var end = SkipFirst ? (Data.Length - 1) | 1 : Data.Length / 2 * 2;   // if there is an odd number of coeffs, throw away the highest one
      var i = start;
      ...
      while (i < end)
      {
        ...
        while (chunkBits < 24 && i < end)
        {
          ...
          var bitTableIndex = coeff1 * 3 + coeff2;
    ...
    public static byte[] EncodeMod3Tight(int[] Data)
    {
      ...
      var size = (THREE.Pow(Data.Length).BitLength + 7) / 8;
      var arr = sum.ToByteArray();
    ...
    {
      var bitsPerCoeff = 31 - IntUtils.NumberOfLeadingZeros(Q);
      var numBits = A.Length * bitsPerCoeff;
      var numBytes = (numBits + 7) / 8;
      var data = new byte[numBytes];
      var bitIndex = 0;
      var byteIndex = 0;
      ...
      {
        for (var j = 0; j < bitsPerCoeff; j++)
        {
          var currentBit = (A[i] >> j) & 1;
