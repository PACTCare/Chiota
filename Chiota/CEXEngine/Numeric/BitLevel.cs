...
namespace VTDev.Libraries.CEXEngine.Numeric
{
    ...
    internal sealed class BitLevel
    {
        ...
        internal static int BitCount(BigInteger Value)
        {
            ...
            if (Value.m_sign > 0)
            {
                for (; i < Value.m_numberLength; i++)
                    bCount += IntUtils.BitCount(Value.m_digits[i]);
            }
            else
            {
                ...
                bCount = (Value.m_numberLength << 5) - bCount;
            }
            return bCount;
        }
        ...
        internal static int BitLength(BigInteger Value)
        {
            ...
            int bLength = (Value.m_numberLength << 5);
            int highDigit = Value.m_digits[Value.m_numberLength - 1];

            if (Value.m_sign < 0)
            {
                int i = Value.FirstNonzeroDigit;
                // We reduce the problem to the positive case.
                if (i == Value.m_numberLength - 1)
                    highDigit--;
            }
        ...
        {
            int resSign = (Value.m_sign == 0) ? 1 : Value.m_sign;
            int intCount = N >> 5;
            int bitN = N & 31;
            int resLength = System.Math.Max(intCount + 1, Value.m_numberLength) + 1;
            int[] resDigits = new int[resLength];
            int i;

            int bitNumber = 1 << bitN;
            Array.Copy(Value.m_digits, 0, resDigits, 0, Value.m_numberLength);

            if (Value.m_sign < 0)
            {
                if (intCount >= Value.m_numberLength)
                {
        ...
        internal static void InplaceShiftLeft(BigInteger Value, int N)
        {
            ...
            Value.m_numberLength += intCount + (IntUtils.NumberOfLeadingZeros(Value.m_digits[Value.m_numberLength - 1]) - (N & 31) >= 0 ? 0 : 1);
            ShiftLeft(Value.m_digits, Value.m_digits, intCount, N & 31);
        ...
        internal static void InplaceShiftRight(BigInteger Value, int N)
        {
            ...
            int intCount = N >> 5; // count of integers
            Value.m_numberLength -= intCount;
            if (!ShiftRight(Value.m_digits, Value.m_numberLength, Value.m_digits, intCount, N & 31) && sign < 0)
            {
        ...
        internal static BigInteger ShiftLeft(BigInteger Value, int N)
        {
            int intCount = N >> 5;
            N &= 31; // %= 32
            int resLength = Value.m_numberLength + intCount + ((N == 0) ? 0 : 1);
            int[] resDigits = new int[resLength];

            ShiftLeft(resDigits, Value.m_digits, intCount, N);
            BigInteger result = new BigInteger(Value.m_sign, resLength, resDigits);
            result.CutOffLeadingZeroes();
            return result;
        }
        ...
        {
            int srcLen = Value.m_numberLength;
            int resLen = srcLen + 1;
            int[] resDigits = new int[resLen];
            ShiftLeftOneBit(resDigits, Value.m_digits, srcLen);
            BigInteger result = new BigInteger(Value.m_sign, resLen, resDigits);
            result.CutOffLeadingZeroes();
        ...
        internal static BigInteger ShiftRight(BigInteger Value, int N)
        {
            ...
            if (intCount >= Value.m_numberLength)
                return ((Value.m_sign < 0) ? BigInteger.MinusOne : BigInteger.Zero);

            int i;
            int resLength = Value.m_numberLength - intCount;
            int[] resDigits = new int[resLength + 1];

            ShiftRight(resDigits, resLength, Value.m_digits, intCount, N);
            if (Value.m_sign < 0)
            {
                // Checking if the dropped bits are zeros (the remainder equals to 0)
                for (i = 0; (i < intCount) && (Value.m_digits[i] == 0); i++)
                {
                    ;
                }
                // If the remainder is not zero, add 1 to the result
                if ((i < intCount)
                        || ((N > 0) && ((Value.m_digits[i] << (32 - N)) != 0)))
                {
        ...
        internal static bool TestBit(BigInteger Value, int N)
        {
            ...
            return ((Value.m_digits[N >> 5] & (1 << (N & 31))) != 0);
        }
        #endregion
    }
}