namespace Chiota.Services.Iota
{
    ...
    {
        public static async Task/*<List<Message>>*/ GetMessages(IAsymmetricKeyPair keyPair, Contact contact)
        {
            var messages = await DependencyResolver.Resolve<IMessenger>().GetMessagesByAddressAsync(new Address(contact.ChatAddress), new MessageBundleParser());
