namespace Chiota.Services.Iota
{
  ...
  public static class UserDataOnTangle
  {
    public static async Task<User> CheckPublicKeyForSingularity(User user)
    {
      var publicKeyList = await IotaHelper.GetPublicKeysAndContactAddresses(user.PublicKeyAddress);
      ...
      {
        await DependencyResolver.Resolve<IMessenger>().SendMessageAsync(new Message(requestAdressTrytes, new Address(user.PublicKeyAddress)));
      }
